...
ggplot(data = Western_Europe, aes(x = Year, y = homicides_per_100k))+
  ...
  labs(title = "Homicide rate in Europe over time",
       y = "Homicide rate in Europe over long-term (per 100,000)")
```
...
**Nedstående visualisering viser at tendensen er, at danske regenters regeringstiden stiger gennem historien. Dette stemmer godt over ens med ovenstående grafer, som viser at mord-raten i Skandinavien og andre store dele af Europa falder gennem historien.**
```{r Midter-året for hver regents regeringstid, eval = TRUE}
library(tidyverse)
read_csv("data/Dansk kongerække - Ark1.csv")
...
Kongerækken <- Kongerækken %>%
  mutate(Regeringstid = Regeringslut - Regeringsstart) %>% 
  mutate(MidYear = Regeringstid/2)

colnames(Kongerækken)

Kongerækken %>% 
  ggplot(aes(x = Død, y = MidYear))+
  geom_smooth()+
  labs(title = "Danske regenters regeringsperiode over tid",
       x = "Død",
       y = "MidYear")
```
...
**- Hvis man sammenholder graferne for mord-raten i store dele af Europa (inkl. Danmark) med visualisering af 'MidYear' for danske regenters regeringsperiode, så er der tale om klar indikation og tendering af et civilisationsudvikling gennem historien. I denne sammenhæng er det vigtig at påpege at der blot er tale om en tendens/indikation. Jeg vil hævde at ovenstående visualiseringer og grafer ikke på fyldstgørende nok. 
Med andre ord er en sammenholden af Europas mord-rate gennem tiden med 'MidYear' for danske regenters regeringstid ikke tilstrækkelig nok til at besvare sådan et åbent og bredt spørgsmål.**